...
test("serves a fresh, published-only Google News sitemap",async()=>{
  ...
  assert.match(newsQuery,/t\.language IN \('en','dv'\)/);assert.match(newsQuery,/t\.content_type IS NULL/);assert.match(newsQuery,/t\.content_type IN \('news','opinion','editorial'\)/);assert.doesNotMatch(newsQuery,/press_release/);assert.match(newsQuery,/t\.article_published_at>=\?/);assert.match(newsQuery,/t\.article_published_at<=\?/);assert.match(newsQuery,/LIMIT \?/);
