...
export type PublicArticle={id:string;language:"en"|"dv";headline:string;summary:string;articleContent:RichTextDocument;imageUrl:string|null;category:string;categoryName:string;breaking:number;important:number;publishedAt:number;articlePublishedAt:number;availableLanguages:Array<"en"|"dv">;relatedGallery:null|{id:string;topic:string;images:Array<{id:string;url:string;sortOrder:number}>}};
...
export async function getPublicArticle(db:D1Database,id:string,language:"en"|"dv"):Promise<PublicArticle|null>{
  ...
  return {...row,articleContent,availableLanguages:languages.results.map(item=>item.language),relatedGallery:gallery?{...gallery,images:JSON.parse(gallery.images)}:null} as PublicArticle;
}
