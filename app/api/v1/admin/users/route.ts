...
export async function GET(request: Request) {
  ...
  const result = await runtime.DB.prepare(`SELECT id,email,display_name AS displayName,role,status,preferred_language AS preferredLanguage,
    ...
    .bind(search, search, ["reader", "admin", "owner"].includes(role ?? "") ? role : null, role, ["active", "suspended"].includes(status ?? "") ? status : null, status).all();
  return Response.json({ actorRole: actor.role, items: result.results });
}
