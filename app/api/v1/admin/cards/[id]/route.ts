...
type Body = { categoryId?: string; imageKey?: string; youtubeUrl?: string|null; sourceName?: string|null; sourceUrl?: string|null; isBreaking?: boolean; isImportant?: boolean; isTimeSensitive?: boolean; status?: "draft"|"archived"; translations?: { en?: TranslationInput; dv?: TranslationInput } };
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const card = await runtime.DB.prepare("SELECT id,category_id AS categoryId,status,image_key AS imageKey,image_url AS imageUrl,youtube_video_id AS youtubeVideoId,source_name AS sourceName,source_url AS sourceUrl,is_breaking AS isBreaking,is_important AS isImportant,is_time_sensitive AS isTimeSensitive,published_at AS publishedAt FROM news_cards WHERE id=?").bind(id).first();
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const videoProvided=Object.prototype.hasOwnProperty.call(body,"youtubeUrl"),videoId=youtubeVideoId(body.youtubeUrl);
  if(videoProvided&&body.youtubeUrl?.trim()&&!videoId)return Response.json({error:{code:"INVALID_YOUTUBE_URL",message:"Enter a valid YouTube video URL"}},{status:400});
  ...
  const runtime = env as unknown as { DB: D1Database; MEDIA: R2Bucket }; const existing = await runtime.DB.prepare("SELECT image_key AS imageKey,youtube_video_id AS youtubeVideoId FROM news_cards WHERE id=?").bind(id).first<{imageKey:string|null;youtubeVideoId:string|null}>();
  ...
  const finalImageKey=body.imageKey??existing.imageKey,finalVideoId=videoProvided?videoId:existing.youtubeVideoId;
  if(!finalImageKey&&!finalVideoId)return Response.json({error:{code:"VISUAL_REQUIRED",message:"Add a photo or YouTube video"}},{status:400});
  const now=Date.now(),requestedPublished=(["en","dv"] as const).some(language=>{const t=body.translations?.[language];return !!t?.published&&!!t.headline?.trim()&&!!t.summary?.trim()}),nextStatus=body.status==="archived"?"archived":requestedPublished?"published":body.status??null; const statements=[runtime.DB.prepare(`UPDATE news_cards SET category_id=COALESCE(?,category_id),image_key=COALESCE(?,image_key),image_url=CASE WHEN ? IS NULL THEN image_url ELSE ? END,youtube_video_id=CASE WHEN ? THEN ? ELSE youtube_video_id END,
    source_name=?,source_url=?,is_breaking=COALESCE(?,is_breaking),is_important=COALESCE(?,is_important),is_time_sensitive=COALESCE(?,is_time_sensitive),status=COALESCE(?,status),published_at=CASE WHEN ?='published' THEN COALESCE(published_at,?) WHEN ?='draft' THEN NULL ELSE published_at END,updated_at=? WHERE id=?`).bind(body.categoryId??null,body.imageKey??null,body.imageKey??null,body.imageKey?`/api/v1/media/${body.imageKey}`:null,videoProvided?1:0,videoId,body.sourceName?.trim().slice(0,100)||null,body.sourceUrl?.trim()||null,typeof body.isBreaking==="boolean"?(body.isBreaking?1:0):null,typeof body.isImportant==="boolean"?(body.isImportant?1:0):null,typeof body.isTimeSensitive==="boolean"?(body.isTimeSensitive?1:0):null,nextStatus,nextStatus,now,nextStatus,now,id)];
  for (const language of ["en","dv"] as const) { const t=body.translations?.[language]; if (!t?.headline?.trim()) continue; const articleProvided=Object.prototype.hasOwnProperty.call(t,"articleContent"),reviewStatus=t.published?"published":"draft",publishedAt=t.published?now:null;statements.push(runtime.DB.prepare(`INSERT INTO news_card_translations (id,card_id,language,headline,summary,word_count,article_content,review_status,published_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)
    ON CONFLICT(card_id,language) DO UPDATE SET headline=excluded.headline,summary=excluded.summary,word_count=excluded.word_count,article_content=CASE WHEN ? THEN excluded.article_content ELSE news_card_translations.article_content END,review_status=excluded.review_status,published_at=CASE WHEN excluded.review_status='published' THEN COALESCE(news_card_translations.published_at,excluded.published_at) ELSE NULL END,article_status=CASE WHEN excluded.review_status='draft' OR (? AND excluded.article_content IS NULL) THEN 'draft' ELSE news_card_translations.article_status END,article_published_at=CASE WHEN excluded.review_status='draft' OR (? AND excluded.article_content IS NULL) THEN NULL ELSE news_card_translations.article_published_at END,updated_at=excluded.updated_at`).bind(crypto.randomUUID(),id,language,t.headline.trim(),t.summary!.trim(),wordCount(t.summary!),articles[language]?JSON.stringify(articles[language]):null,reviewStatus,publishedAt,now,now,articleProvided?1:0,articleProvided?1:0,articleProvided?1:0)); }
