...
type Body = { categoryId?: string; imageKey?: string; sourceName?: string|null; sourceUrl?: string|null; isBreaking?: boolean; status?: "draft"|"archived"; translations?: { en?: TranslationInput; dv?: TranslationInput } };
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const card = await runtime.DB.prepare("SELECT id,category_id AS categoryId,status,image_key AS imageKey,image_url AS imageUrl,source_name AS sourceName,source_url AS sourceUrl,is_breaking AS isBreaking,published_at AS publishedAt FROM news_cards WHERE id=?").bind(id).first();
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const now=Date.now(); const statements=[runtime.DB.prepare(`UPDATE news_cards SET category_id=COALESCE(?,category_id),image_key=COALESCE(?,image_key),image_url=CASE WHEN ? IS NULL THEN image_url ELSE ? END,
    source_name=?,source_url=?,is_breaking=COALESCE(?,is_breaking),status=COALESCE(?,status),updated_at=? WHERE id=?`).bind(body.categoryId??null,body.imageKey??null,body.imageKey??null,body.imageKey?`/api/v1/media/${body.imageKey}`:null,body.sourceName?.trim().slice(0,100)||null,body.sourceUrl?.trim()||null,typeof body.isBreaking==="boolean"?(body.isBreaking?1:0):null,body.status??null,now,id)];
