...
type Body = { categoryId?: string; imageKey?: string; sourceName?: string; sourceUrl?: string; isBreaking?: boolean; isImportant?: boolean; isTimeSensitive?: boolean; translations?: { en?: TranslationInput; dv?: TranslationInput } };
...
export async function GET(request: Request) {
  ...
  const rows = await runtime.DB.prepare(`SELECT c.id,c.status,c.image_key AS imageKey,c.source_name AS sourceName,c.source_url AS sourceUrl,c.is_breaking AS isBreaking,c.is_important AS isImportant,c.is_time_sensitive AS isTimeSensitive,c.published_at AS publishedAt,
...
export async function POST(request: Request) {
  ...
  await runtime.DB.batch([
    runtime.DB.prepare("INSERT INTO news_cards (id,category_id,status,image_key,image_url,source_name,source_url,is_breaking,is_important,is_time_sensitive,published_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)")
      .bind(id,body.categoryId,published?"published":"draft",body.imageKey,`/api/v1/media/${body.imageKey}`,body.sourceName?.trim().slice(0,100)||null,body.sourceUrl?.trim()||null,body.isBreaking?1:0,body.isImportant?1:0,body.isTimeSensitive?1:0,published?now:null,now,now),
