...
type Body = { categoryId?: string; imageKey?: string; sourceName?: string; sourceUrl?: string; translations?: { en?: TranslationInput; dv?: TranslationInput } };
...
export async function GET(request: Request) {
  ...
  const rows = await runtime.DB.prepare(`SELECT c.id,c.status,c.image_key AS imageKey,c.source_name AS sourceName,c.source_url AS sourceUrl,c.published_at AS publishedAt,
...
export async function POST(request: Request) {
  ...
  await runtime.DB.batch([
    runtime.DB.prepare("INSERT INTO news_cards (id,category_id,status,image_key,image_url,source_name,source_url,published_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)")
      .bind(id,body.categoryId,published?"published":"draft",body.imageKey,`/api/v1/media/${body.imageKey}`,body.sourceName?.trim().slice(0,100)||null,body.sourceUrl?.trim()||null,published?now:null,now,now),
    ...
    runtime.DB.prepare("INSERT INTO audit_events (id,actor_id,action,entity_type,entity_id,after,created_at) VALUES (?,?,?,?,?,?,?)").bind(crypto.randomUUID(),actor.id,"card.created","news_card",id,JSON.stringify({languages:present}),now),
  ]);
