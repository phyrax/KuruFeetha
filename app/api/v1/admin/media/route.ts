...
export async function DELETE(request: Request) {
  ...
  const used = await runtime.DB.prepare("SELECT 1 FROM news_cards WHERE image_key=? UNION ALL SELECT 1 FROM gallery_images WHERE image_key=? LIMIT 1").bind(key,key).first();
