...
import{OpenAIContentTypeProvider}from"../../../../../lib/openai-content-type-provider";
...
export async function POST(request:Request){
  ...
  const results=await mapLimit(ids,3,async storyId=>{try{return{ok:true,storyId,recommendation:await analyzeClassifierStory(db,provider,storyId,{force:Boolean(body?.force)})}}catch(error){return{ok:false,storyId,error:(error as Error).message}}});return Response.json({results});
}
