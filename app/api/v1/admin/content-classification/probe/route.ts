...
export async function POST(request:Request){
  ...
  try{const result=body.probe==="plain"?await provider.probePlain():body.probe==="structured"?await provider.probeStructured():await provider.probeClassifierSchema();if(body.probe==="classifier_schema")console.info("OPENAI_CLASSIFIER_PROBE_SUCCESS",result);return Response.json({ok:true,result})}catch(error){const diagnostic=providerDiagnostic(error);if(diagnostic){console.error("OPENAI_PROVIDER_FAILURE",diagnostic);return Response.json({ok:false,diagnostic})}console.error("OPENAI_PROVIDER_FAILURE",{diagnosticCode:"OPENAI_PROVIDER_FAILURE",provider:"openai",model:provider.model,message:"Unexpected provider diagnostic failure"});return Response.json({ok:false,diagnostic:{diagnosticCode:"OPENAI_PROVIDER_FAILURE",provider:"openai",model:provider.model,httpStatus:null,errorType:null,errorCode:null,message:"Unexpected provider diagnostic failure",requestId:null,responseStatus:null,incompleteReason:null}})}
}
