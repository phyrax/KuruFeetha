...
type Body={topic?:string;language?:"en"|"dv";categoryId?:string;relatedStoryId?:string|null;imageKeys?:string[];published?:boolean};
...
export async function GET(request:Request){
  ...
  const result=await runtime.DB.prepare(`SELECT g.id,g.topic,g.language,g.category_id AS categoryId,cat.name_en AS categoryEn,cat.name_dv AS categoryDv,g.related_story_id AS relatedStoryId,g.status,g.published_at AS publishedAt,
    json_group_array(json_object('id',i.id,'key',i.image_key,'url',i.image_url,'sortOrder',i.sort_order)) AS images
    FROM galleries g LEFT JOIN categories cat ON cat.id=g.category_id LEFT JOIN gallery_images i ON i.gallery_id=g.id GROUP BY g.id ORDER BY g.updated_at DESC LIMIT 100`).all();
...
export async function POST(request:Request){
  ...
  if(!await runtime.DB.prepare("SELECT 1 FROM categories WHERE id=? AND enabled=1").bind(body!.categoryId).first())return Response.json({error:{code:"INVALID_CATEGORY",message:"Choose an enabled category"}},{status:400});
  ...
  const id=crypto.randomUUID(),now=Date.now(),statements=[runtime.DB.prepare("INSERT INTO galleries (id,topic,language,category_id,related_story_id,status,published_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)").bind(id,body!.topic!.trim(),body!.language,body!.categoryId,body!.relatedStoryId||null,body!.published?"published":"draft",body!.published?now:null,now,now)];
